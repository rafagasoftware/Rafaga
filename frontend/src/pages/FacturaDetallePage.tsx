...
import { Alert, Box, Button, Chip, Divider, Grid, Paper, Stack, Table, TableBody, TableCell, TableHead, TableRow, Tooltip, Typography } from '@mui/material';
...
import { calcularSubtotalItem, calcularTotales, formatearMoneda } from './facturar/calculos';
import type { ItemFactura } from './facturar/types';
...
export function FacturaDetallePage() {
  ...
  const { lote, cliente } = factura;
  const items: ItemFactura[] = lote.lote_items.map((row) => ({
    id: row.id,
  ...
  const tipoComprobante = TIPOS_COMPROBANTE.find((t) => t.value === lote.tipo_comprobante);
      ...
      <Paper variant="outlined" sx={{ p: 4, maxWidth: 800 }}>
        <Grid container spacing={2} sx={{ mb: 2 }}>
          ...
          <Grid size={5} sx={{ textAlign: 'right' }}>
            ...
            <Typography variant="body2" sx={{ fontVariantNumeric: 'tabular-nums' }}>
              {lote.punto_venta ? String(lote.punto_venta.numero).padStart(4, '0') : '----'}-
              {factura.numero_comprobante ?? 'pendiente'}
            </Typography>
            <Typography variant="body2" color="text.secondary">
              Emisión {new Date(lote.fecha_emision).toLocaleDateString('es-AR')}
            </Typography>
          </Grid>
        </Grid>
        ...
        <Grid container spacing={2} sx={{ mb: 2 }}>
          ...
          <Grid size={6} sx={{ textAlign: 'right' }}>
            ...
            {lote.periodo_desde && (
              <Typography variant="body2" color="text.secondary">
                Período: {lote.periodo_desde} al {lote.periodo_hasta}
              </Typography>
            )}
            {lote.vencimiento_pago && (
              <Typography variant="body2" color="text.secondary">
                Vencimiento de pago: {lote.vencimiento_pago}
              </Typography>
            )}
          </Grid>
        </Grid>
        ...
        <Paper variant="outlined" sx={{ p: 2, mb: 3, bgcolor: 'background.default' }}>
          ...
          <Typography variant="body1">{cliente.razon_social}</Typography>
          <Typography variant="body2" color="text.secondary">
            {cliente.tipo_documento} {cliente.numero_documento} · {cliente.condicion_iva}
          </Typography>
          {cliente.domicilio && (
            <Typography variant="body2" color="text.secondary">
              {cliente.domicilio}
